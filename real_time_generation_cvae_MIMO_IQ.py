...
from conditional_vae_with_diffraction import Conditional_VAE_MIMO_IQ
from utils_diffraction_model import generate_conditioned_attenuation_sample_binlabels_MIMO_IQ_test
...
parser.add_argument("-latent_dim", default=32, help="set the number of latent dimensions for reconstruction", type=int)
parser.add_argument("-beta", default=0.00001, help="set the beta weighting for KL divergence", type=float)
...
parser.add_argument("-gen", default=50, help="set the number of generated samples", type=int)
parser.add_argument("-height", default=1.64, help="set the target height", type=float)
...
checkpoint_path = "training_cvae_EM_{}_{}_heights_{}_dimensions{}_bin_labels_MIMO_IQ/cvae.ckpt".format(latent_dim, beta,
                                                                                                      num_heights,
                                                                                                      num_dim)
...
if __name__ == '__main__':
    ...
    model = Conditional_VAE_MIMO_IQ(latent_dim, num_labels, rotation_points, links,2)
    # filename = 'EM_norm_parameters_{}.mat'.format(num_classes)
    filename = 'EM_norm_parameters_{}_generation_150_MIMO_IQ.mat'.format(num_classes)
    ...
    if random_height_dim == 1:
        ...
        csi_responses = np.zeros((2*rotation_points-1 , links, 2, generation, num_positions)) # generated attenuations (rotations, number of generations, num positions)
        positions_g = np.zeros((num_positions, generation, 2))
        height = args.height
        tm = args.tm
        for pos in range(num_positions):
            ...
            csi_responses[:, :, :, :,  pos], positions_g[pos, :, :] = generate_conditioned_attenuation_sample_binlabels_MIMO_IQ_test(args, model, dataset_max, dataset_mean, dataset_std, rotation_points, rotations, num_labels, nominal_positions, position, deviation, step_distance, step_distance2, generation)
            # positions[pos*generation:generation,:] = position_vec

        dict_1 = {"generated_CSI": csi_responses, "rotations": rotations,
                  "generation": generation,
                  "nominal_positions": nominal_positions,
                  "true_positions": positions_g,
                  "deviation": deviation, "step_distance": step_distance}

        sio.savemat(
            "results/generated_samples_cvae_latent_vars_{}_beta_{}_height_{}_targetdim_{}_binlabels_MIMO_IQ.mat".format(latent_dim, beta,
                                                                                                      height, tm), dict_1)
        # not implemented
    elif random_height_dim == 2:
        ########### generate positions with an assigned target dimension and position
        att_responses = np.zeros((generation, 2*rotation_points - 1, links, 2, 1)) # generated attenuations (rotations, number of generations)
        height = args.height
        tm = args.tm
        position = [args.pos_x, args.pos_y]
        deviation = 0 # disable position deviation around nominal location
        att_responses[:, :, :, :, 0] = generate_conditioned_attenuation_sample_binlabels_MIMO_IQ_test(args, model, dataset_max, dataset_mean,
        ...
        sio.savemat(
            "results/generated_samples_cvae_latent_vars_{}_beta_{}_height_{}_targetdim_{}_position{}_binlabels_MIMO_IQ.mat".format(latent_dim, beta,
